...
static ssize_t mem_rw(struct file *file, char __user
...
if (write &&
copy_from_user(page, buf, this_len
)) {
copied = -EFAULT;
break;
}
...
if (!write &&
copy_to_user(buf, page, this_len
)) {
copied = -EFAULT;
break;
}
...
static ssize_t mem_read(struct file *file, char __user
...
static ssize_t oom_adj_read(struct file *file, char __user
...
return
simple_read_from_buffer(buf, count, ppos, buffer, len
);
}

static ssize_t oom_adj_write(struct file *file, const char __user
...
static ssize_t oom_score_adj_read(struct file *file, char __user
...
static ssize_t oom_score_adj_write(struct file *file, const char __user
...
task->signal->
oom_score_adj = (short) oom_score_adj;
if (
has_capability_noaudit(current, CAP_SYS_RESOURCE
))
task->signal->
oom_score_adj_min = (short) oom_score_adj;
trace_oom_score_adj_update(task);
...
static ssize_t comm_write(struct file *file, const char __user
...
static int do_proc_readlink(struct path *path, char __user
...
static int proc_pid_readlink(struct dentry *dentry, char __user
